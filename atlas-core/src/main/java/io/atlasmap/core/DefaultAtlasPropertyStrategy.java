...
public class DefaultAtlasPropertyStrategy implements AtlasPropertyStrategy {
	...
	public void processPropertyField(AtlasMapping atlasMapping, PropertyField propertyField, Map<String, Object> runtimeProperties) throws AtlasUnsupportedException, AtlasConversionException {
        ...
        for(AtlasPropertyType propType : getPropertyOrder()) {
            switch(propType) {
            case ENVIRONMENT_VARIABLES: processEnvironmentVariable(propertyField); break;
            case JAVA_SYSTEM_PROPERTIES: processJavaSystemProperty(propertyField); break;
            case MAPPING_DEFINED_PROPERTIES: processMappingDefinedProperties(propertyField, atlasMapping); break;
            case RUNTIME_PROPERTIES: processRuntimeProperties(propertyField, runtimeProperties); break;
            default: throw new AtlasUnsupportedException(String.format("Unsupported PropertyType detected type=%s for field=%s", propType, AtlasModelFactory.toString(propertyField)));
            }
        }
    ...
    protected void processEnvironmentVariable(PropertyField propertyField) {
        ...
        try {
            ...
            logger.error(String.format("SecurityException while processing environment variable for propertyField=%s", AtlasModelFactory.toString(propertyField)), e);
        }
    }
    ...
    protected void processJavaSystemProperty(PropertyField propertyField) {
        ...
        try {
            ...
            logger.error(String.format("SecurityException while processing Java system property for propertyField=%s", AtlasModelFactory.toString(propertyField)), e);
        }
    }
    ...
    protected void processMappingDefinedProperties(PropertyField propertyField, AtlasMapping atlasMapping) throws AtlasConversionException {
        ...
        for(Property prop : atlasMapping.getProperties().getProperty()) {
            if(propertyField.getName().equals(prop.getName())) {
                if(getAtlasConversionService() != null && (propertyField.getFieldType() != null || prop.getFieldType() != null)) {
                    propertyField.setValue(getAtlasConversionService().convertType(prop.getValue(), FieldType.STRING, (propertyField.getFieldType() != null ? propertyField.getFieldType() : prop.getFieldType())));
                    if(logger.isDebugEnabled()) {
        ...
        for(String key : runtimeProperties.keySet()) {
            if(propertyField.getName().equals(key)) {
                if(getAtlasConversionService() != null && propertyField.getFieldType() != null) {
                    propertyField.setValue(getAtlasConversionService().convertType(runtimeProperties.get(key), getAtlasConversionService().fieldTypeFromClass(runtimeProperties.get(key).getClass()), propertyField.getFieldType()));
                    if(logger.isDebugEnabled()) {
