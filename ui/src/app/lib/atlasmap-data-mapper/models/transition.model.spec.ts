...
describe('FieldActionConfig.appliesToField()', () => {
    ...
    it('should return false if FieldMappingPair is null', () => {
      expect(action.appliesToField(null)).toBe(false);
    });

    it('should return false if source or target is null', () => {
      pair.targetFields.splice(0);
      expect(action.appliesToField(pair)).toBe(false);
    });
    ...
    it('should return if action target type is NUMBER and target field type is numeric', () => {
      ...
      target.type = 'SHORT';
      expect(action.appliesToField(pair)).toBe(true);
      target.type = 'NUMBER';
      expect(action.appliesToField(pair)).toBe(true);
      target.type = 'STRING';
      expect(action.appliesToField(pair)).toBe(false);
    ...
    it('should return if action source type is ANY or matches target field type, and action target type matches target type',
       () => {
      target.type = 'STRING';
