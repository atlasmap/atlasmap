...
export interface IImportActionProps {
  id: string;
  onImport: (selectedFile: File, docType: DocumentType) => void;
}
...
const importCandidate: importDocumentType[] = [
  {
    documentType: DocumentType.JSON_SCHEMA,
    typeName: 'JSON schema',
    suffix: '.json',
  },
  {
    documentType: DocumentType.JSON_INSTANCE,
    typeName: 'JSON instance',
    suffix: '.json',
  },
  {
    documentType: DocumentType.XML_SCHEMA,
    typeName: 'XML schema or AtlasMap XML SchemaSet',
    suffix: '.xml, .xsd',
  },
  {
    documentType: DocumentType.XML_INSTANCE,
  ...
  {
    documentType: DocumentType.KAFKA_JSON_SCHEMA,
    typeName: 'Kafka Connect JSON schema',
    suffix: '.json, .txt, ""',
  },
  {
    documentType: DocumentType.KAFKA_AVRO_SCHEMA,
...
export const ImportDocumentTypeItem: FunctionComponent<{
  ...
  onFile: (file: File, docType: DocumentType) => void;
  onImport: (file: File, docType: DocumentType) => void;
  ...
  useEffect(() => {
    if (previouslyUploadedFiles.current !== files) {
      ...
      if (files?.length === 1) {
        ...
        onFile(files[0], importCandidate[importDocumentTypeIndex].documentType);
      }
    }
