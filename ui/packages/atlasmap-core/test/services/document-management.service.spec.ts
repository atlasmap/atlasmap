...
describe('DocumentManagementService', () => {
  ...
  test('parse Java inspection', () => {
    ...
    docDef.inspectionResult = JSON.stringify(atlasmapInspectionTwitter4jStatusJson);
    service.fetchDocument(docDef, '').subscribe((answer: DocumentDefinition) => {
      ...
      const screenName = user?.children?.filter((child: Field) => child?.name === 'screenName');
      if (!screenName) {
        fail('no screenName');
      }
  ...
  test('parse JSON inspection', () => {
    ...
    docDef.inspectionResult = JSON.stringify(atlasmapInspectionComplexObjectRootedJson);
    service.fetchDocument(docDef, '').subscribe((answer: DocumentDefinition) => {
  ...
  test('parse XML inspection', () => {
    ...
    docDef.inspectionType = InspectionType.SCHEMA;
    docDef.inspectionResult = JSON.stringify(atlasmapInspectionPoExampleSchemaJson);
    service.fetchDocument(docDef, '').subscribe((answer: DocumentDefinition) => {
      expect(answer.fields.length).toBe(1);
