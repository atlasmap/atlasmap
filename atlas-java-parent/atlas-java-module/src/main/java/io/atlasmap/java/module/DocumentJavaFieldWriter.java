...
public class DocumentJavaFieldWriter {	 	
    ...
    public void write(Field field, JavaFieldWriterValueConverter converter) throws AtlasException {
    	try {    		
	        ...
	        if (logger.isDebugEnabled()) {    		
	    		...
	    		String fieldClassName = (field instanceof JavaField) ? ((JavaField)field).getClassName() : ((JavaEnumField)field).getClassName();
    ...
    public Object findChildObject(Field field, SegmentContext segmentContext, Object parentObject) throws AtlasException {
    	...
    	if (logger.isDebugEnabled()) {
    		logger.debug("Looking for child object '" + segment + "' in parent '" + parentSegment + "': " + writeDocumentToString(false, parentObject));
    	}    	
    	...
    	if (childObject != null && PathUtil.isCollectionSegment(segment)) {
    		if (!collectionHasRoomForIndex(childObject, segmentContext)) {
    			if (logger.isDebugEnabled()) {
    				logger.debug("Found child collection '" + segment + "' (" + childObject.getClass().getName() + ") in parent '" + parentSegment 
        					+ "', but it doesn't have room for the segment's index. Parent Object: " + writeDocumentToString(false, parentObject));
    			}
    			return null;
    		}
    	...
    	if (logger.isDebugEnabled()) {
    		if (childObject == null) {
    			...
    			logger.debug("Found child object '" + segment + "' in parent '" + parentSegment 
    					+ "', class: " + childObject.getClass().getName() + ", child object: " + writeDocumentToString(false, childObject));
    		}    		
    	}
    ...
    public Object createParentObject(Field field, Object parentObject, SegmentContext segmentContext) throws AtlasException {
    	String segment = segmentContext.getSegment();
    	if (logger.isDebugEnabled()) {
		...
		if (PathUtil.isCollectionSegment(segment)) {			
			...
			if (logger.isDebugEnabled()) {
				logger.debug("Child object inside collection wrapper for segment '" + segment + "': " + writeDocumentToString(false, childObject));
			}
		} else {
			childObject = createObject(field, segmentContext, parentObject, false);
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Created child object for segment '" + segment + "': " + writeDocumentToString(true, childObject));
		}
		return childObject;
    }    
    
    public Object findOrCreateOrExpandParentCollectionObject(Field field, Object parentObject, SegmentContext segmentContext) throws AtlasException {
    	String segment = segmentContext.getSegment();
    	//first, let's see if we have the collection object at all
		if (logger.isDebugEnabled()) {
			logger.debug("Looking for collection wrapper child for " + segmentContext + " on parent: " + parentObject);
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Collection wrapper child object for segment '" + segment + "': " + writeDocumentToString(false, collectionObject));
		}			
    ...
    public Object expandCollectionToFitItem(Field field, Object collectionObject, SegmentContext segmentContext, Object parentObject) throws AtlasException {
    	String segment = segmentContext.getSegment();
	...
	public Object createCollectionWrapperObject(Field field, SegmentContext segmentContext, Object parentObject) throws AtlasException {
	...
	public Class<?> getClassForField(Field field, SegmentContext segmentContext, Object parentObject, boolean unwrapCollectionType) throws AtlasException {
		if (logger.isDebugEnabled()) {
		...
		if (clz == null) { //attempt to determine it from the parent object.
			...
			Method m = null;
			try {
				String methodName = "get" + JavaWriterUtil.capitalizeFirstLetter(PathUtil.cleanPathSegment(segmentContext.getSegment()));
    	...
    	if (unwrapCollectionType && clz.isArray()) {
    		...
    		if (logger.isDebugEnabled()) {
    			logger.debug("Unwrapped type '" + clz.getName() + "' from wrapper array type '" + oldClass.getName() + "'.");
    		}
    		...
    		for (java.lang.reflect.Field declaredField : parentObject.getClass().getDeclaredFields()) {
    			if (cleanedSegment.equals(declaredField.getName())) {
    				if (declaredField.getGenericType() == null) {
    					if (logger.isDebugEnabled()) {
    						logger.debug("Skipping determining generic type declared field '" + declaredField.getName() + "', the field isn't generic. Segment: " + segmentContext);
    					}
    					continue;
    				}
    		...
    		if (logger.isDebugEnabled()) {
    			logger.debug("Unwrapped type '" + clz.getName() + "' from wrapper list type '" + oldClass.getName() + "'.");
    		}
    	}
	...
	public Object createObject(Field javaField, SegmentContext segmentContext, Object parentObject, boolean createWrapperArray) throws AtlasException {				
		return writerUtil.instantiateObject(getClassForField(javaField, segmentContext, parentObject, true), segmentContext, createWrapperArray);
	}
    ...
    @SuppressWarnings({ "rawtypes", "unchecked" })
	public void addChildObject(Field field, SegmentContext segmentContext, Object parentObject, Object childObject) throws AtlasException {
    	if (logger.isDebugEnabled()) {
