...
public class JavaConstructServiceTest {
    ...
    public void testConstructClassCollectionArray() throws Exception {
        Object stringArray = constructService.constructClass(generateJavaClassCollection("java.lang.String", CollectionType.ARRAY, new Integer(1), new Integer(3)));
    ...
    public void testConstructClassCollectionUnsupported() throws Exception {               
        try {
            constructService.constructClass(generateJavaClassCollection("java.lang.String", CollectionType.ALL, null, null));
            fail("ConstructUnsupportedException expected");
        } catch (ConstructUnsupportedException e) {
            assertEquals(String.format("Unsupported collectionType for instantiation c=%s cType=%s", "java.lang.String", CollectionType.ALL.value()), e.getMessage());
        } catch (Exception e) {
            fail("Expected ConstructException instead: " + e.getMessage());
        }
        
        try {
            constructService.constructClass(generateJavaClassCollection("java.lang.String", CollectionType.NONE, null, null));
            fail("ConstructUnsupportedException expected");
        } catch (ConstructUnsupportedException e) {
            assertEquals(String.format("Unsupported collectionType for instantiation c=%s cType=%s", "java.lang.String", CollectionType.NONE.value()), e.getMessage());
        } catch (Exception e) {
            fail("Expected ConstructException instead: " + e.getMessage());
        }
    }
