...
public class JavaPath {
    ...
    public JavaPath getLastSegmentParentPath() {
        if (segments.size() == 0 || segments.size() == 1) {
            return null;
        }

        JavaPath parentPath = new JavaPath();
    ...
    public JavaPath deCollectionify(String collectionSegment) {
        if (segments.size() == 0 || segments.size() == 1) {
            return null;
        }

        JavaPath j = new JavaPath();
    ...
    public String getCollectionSegment() {
        for(String part : getSegments()) {
            if(JavaPath.isCollectionSegment(part)) {
                return part;
            }
        }
        return null;
