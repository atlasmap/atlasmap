...
import io.atlasmap.java.test.TestOrder;
...
public class ComplexClassInspectTest {
	...
	public void testComplexClass() throws Exception {
		JavaClass c = classInspectionService.inspectClass(TestOrder.class);
		assertNotNull(c);
		assertNull(c.getAnnotations());
		assertNull(c.getArrayDimensions());
		assertEquals("io.atlasmap.java.test.TestOrder", c.getClassName());
		...
		assertEquals(String.format(AtlasJavaModelFactory.URI_FORMAT, "io.atlasmap.java.test.TestOrder"), c.getUri());
