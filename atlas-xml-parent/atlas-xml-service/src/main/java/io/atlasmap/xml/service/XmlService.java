    ...
    public Response inspectClass(XmlInspectionRequest request) throws Exception {
    	...
    	try {
    	    ...
    	    if(request.getType() == null) {
    	        ...
    	        case INSTANCE:
    	            d = s.inspectXmlDocument(request.getXmlData()); break;
    	        case SCHEMA:
    	            d = s.inspectSchema(request.getXmlData()); break;
    	        default: response.setErrorMessage("Unsupported inspection type: " + request.getType()); break;
    	        }
    	    }    		
    	} catch (Exception e) {
    		logger.error("Error inspecting xml: " + e.getMessage(), e);
