...
* Code Location: https://github.com/atlasmap/atlasmap/blob/main/lib/
...
--------------------------------------------------------------------
<1> https://github.com/atlasmap/atlasmap/blob/main/lib/api/src/main/java/io/atlasmap/api/AtlasContextFactory.java[AtlasContextFactory] is a singleton instance on JVM, which holds global configuration for the AtlasMap.
<2> `AtlasContextFactory#createContext(File)` creates https://github.com/atlasmap/atlasmap/blob/main/lib/api/src/main/java/io/atlasmap/api/AtlasContext.java[AtlasContext] which represents an AtlasMap mapping context for each mapping definitions by feeding an ADM archive file exported from the AtlasMap Data Mapper UI.
<3> https://github.com/atlasmap/atlasmap/blob/main/lib/api/src/main/java/io/atlasmap/api/AtlasSession.java[AtlasSession] represents a mapping processing session. `AtlasSession` should be created for each execution and should *NOT* be shared among multiple threads.
...
https://github.com/atlasmap/atlasmap/blob/main/lib/api/src/main/java/io/atlasmap/spi/AtlasModule.java[AtlasModule] is a SPI to be implemented by each modules like
https://github.com/atlasmap/atlasmap/tree/main/lib/modules/java[Java], 
https://github.com/atlasmap/atlasmap/tree/main/lib/modules/json[JSON] and
https://github.com/atlasmap/atlasmap/tree/main/lib/modules/xml[XML]. The methods defined in `AtlasModule` are invoked from `AtlasContext` while `AtlasContext#process(AtlasSession)` is in progress.
...
https://github.com/atlasmap/atlasmap/tree/main/lib/core/src/main/java/io/atlasmap/converters[here].
...
https://github.com/atlasmap/atlasmap/tree/main/lib/core/src/main/java/io/atlasmap/actions[here].
